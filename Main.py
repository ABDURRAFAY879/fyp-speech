...
class feature_analysis_graphs:
    ...
    def melspectrogram_graph(self, data):
        ...
        img = librosa.display.specshow(
            S_dB, x_axis="time", y_axis="mel", sr=16000, fmax=8000, ax=ax
        )
...
def makechunk(Inputfilename, sample_rate, foldername):
    ...
    audio_segment = AudioSegment.from_file(Inputfilename)
    ...
    nonsilent_data = detect_nonsilent(
        normalized_sound, min_silence_len=40, silence_thresh=-36, seek_step=1
    )
...
def dtw_calculation(samplefile, targetfile, sample_rate):
    ...
    print("dtw_calculation", sample1, sampletest)
...
def Main(Inputfilename):
    ...
    print("target_chunks", target_chunks)
    status = "Processing"
    distance = []
    ...
    if len(test_chunks) == len(target_chunks):
        ...
        for i in range(len(test_chunks)):
            most_occur = get_large_audio_transcription(test_chunks[i])
            pred_ML_RF, pred_ML_KNN, pred_ML_SVM, pred_ML_Vote = speechrecognitiontest(
                test_chunks[i]
            )
            Result = [
    ...
    print("#" * 50)
    print(Surah_Result, wordsresult, distance, status, chunks_timestamps)
    print("#" * 50)
    return Surah_Result, wordsresult, distance, status, chunks_timestamps
...
if __name__ == "__main__":
    ...
    # Inputfilename = "ayat/سورة الفلق004/113004_31.wav"
    # print(Main(Inputfilename))

#    Inputfilename = "ayat/سورة الفلق003/113003_31.wav"
#    print(Main(Inputfilename))

#   Inputfilename = "ayat/سورة الناس001/114001_01.wav"
#  print(Main(Inputfilename))
